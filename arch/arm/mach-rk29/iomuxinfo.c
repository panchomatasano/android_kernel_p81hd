...
unsigned int rk29_mux_get(struct mux_config *cfg)
{
	int regValue;
	int mask;
	...
	regValue = readl(cfg->mux_reg);
	regValue &= mask;
	cfg->mode = regValue >> cfg->offset;
	return cfg->mode;
}

static int rk29_mux_show(struct seq_file *s, void *v)
{
	int i;
	for (i = 0; i < ARRAY_SIZE(rk29_muxs); i++)
	{
		seq_printf(s, "%s\t%d\n", rk29_muxs[i].name, rk29_mux_get(&rk29_muxs[i]));
	} 
	return 0;
}

static int iomuxinfo_proc_open(struct inode *inode, struct file *file)
{
	return single_open(file, rk29_mux_show, NULL);
}

static struct file_operations iomuxinfo_ops = {
	.owner  = THIS_MODULE,
	.open   = iomuxinfo_proc_open,
        .read   = seq_read,
        .llseek = seq_lseek,
        .release= seq_release,
...
static struct proc_dir_entry *iomuxinfo_entry = NULL;
#define IOMUXINFO_ENTRY_NAME "iomuxinfo"

static int __init iomuxinfo_init(void)
{
	iomuxinfo_entry = create_proc_entry(IOMUXINFO_ENTRY_NAME, 0, NULL);
	if (iomuxinfo_entry)
		iomuxinfo_entry->proc_fops = &iomuxinfo_ops;

	return 0;
}

static void __exit iomuxinfo_exit(void)
{
	remove_proc_entry(IOMUXINFO_ENTRY_NAME, NULL);
}

module_init(iomuxinfo_init)
module_exit(iomuxinfo_exit)